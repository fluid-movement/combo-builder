...
<div class="rounded-xl border-4 border-primary bg-button overflow-hidden">
  ...
  >
    ...
    <div class="grow">
      <p class="font-bold text-xl my-1">{pickedMove.move.name}</p>
      <p class="text-xs text-muted capitalize my-1">
        <span class="font-bold">{pickedMove.asType}</span> · {pickedMove.move
          .level}
      </p>
    </div>
    {#if pickedMove.move.description || pickedMove.move.videoId}
      <ChevronDown
        class="text-primary transition-transform duration-300 shrink-0 {expanded
          ? 'rotate-180'
          : ''}"
      />
    {/if}
  ...
  {#if expanded && (pickedMove.move.description || pickedMove.move.videoId)}
    <div
      ...
      class="px-4 py-2 border-t-2 border-primary/20"
    >
